...
static void display_char_on_7seg(char ch) {
    ...
    int i;
    for (i = 0; i < 7; i++) {
        gpio_set_value(seg_pins[i], (pattern >> i) & 0x01);
    }
}
...
// 接收學號
static ssize_t lab3_write(struct file *file, const char __user *buf, size_t len, loff_t *off) {
...
// 每次讀出一個字元 → 顯示在七段
static ssize_t lab3_read(struct file *file, char __user *buf, size_t len, loff_t *off) {
    char ch;
